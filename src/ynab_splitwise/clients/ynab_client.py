...
class YnabClient(LoggerMixin):
    ...
    def __init__(self, config: Config) -> None:
        ...
        ynab_config = ynab.Configuration(
            host=config.ynab_api_url, access_token=config.ynab_access_token
        )
        ...
        try:
            ...
            for account in accounts_response.data.accounts:
                if account.name == account_name:
                    ...
                    self.logger.info(
                        f"Found account '{account_name}' with ID: {self._account_id}"
                    )
                    return self._account_id
    ...
    def create_transaction(
        ...
        try:
            ...
            self.logger.debug(
                f"Transaction details: amount={amount}, memo='{memo}', import_id='{import_id}'"
            )
            ...
            if response.data.transaction:
                ...
                self.logger.info(
                    f"Successfully created transaction with ID: {created_transaction.id}"
                )
                return {
    ...
    def create_transactions_batch(
        ...
        try:
            ...
            if response.data.transactions:
                ...
                self.logger.info(
                    f"Successfully created {len(created_transactions)} transactions"
                )
    ...
    def get_transactions_by_import_id(self, import_ids: List[str]) -> List[str]:
        ...
        try:
            ...
            self.logger.info(
                f"Checking for existing transactions with {len(import_ids)} import IDs"
            )
