...
class SplitwiseClient(LoggerMixin):
    ...
    def get_current_user(self) -> Dict[str, Any]:
        ...
        user = data["user"]
        self.logger.info(f"Current user: {user.get('first_name', '')} {user.get('last_name', '')} ({user.get('email', '')})")
        return user
    ...
    def get_all_expenses_since(self, start_date: datetime) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"Retrieved total of {len(all_expenses)} expenses since {start_date.isoformat()}")
        return all_expenses
    
    def get_user_share_for_expense(self, expense: Dict[str, Any], user_id: int) -> Dict[str, float]:
