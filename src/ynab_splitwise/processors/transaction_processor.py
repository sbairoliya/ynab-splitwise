...
class TransactionProcessor(LoggerMixin):
    ...
    def _convert_expense_to_transaction(
        ...
        try:
            ...
            if user_share is None:
                ...
                self.logger.debug(f"User {user_id} not involved in expense {expense_id}")
                return None
            ...
            if amount_milliunits == 0:
                ...
                self.logger.debug(f"User {user_id} has no net amount for expense {expense_id}")
                return None
            ...
            expense_id = expense.get("id", "unknown")
            raise DataProcessingError(f"Failed to convert expense {expense_id}", details=str(e))
    ...
    def _parse_expense_date(self, date_str: str) -> datetime:
        ...
        try:
            ...
            raise DataProcessingError(f"Invalid date format: {date_str}", details=str(e))
    
    def _get_user_share(self, expense: Dict[str, Any], user_id: int) -> Dict[str, float] | None:
    ...
    def _generate_memo(self, expense: Dict[str, Any], user_share: Dict[str, float]) -> str:
    ...
    def validate_transactions(self, transactions: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            for i, txn in enumerate(transactions):
                ...
                if not isinstance(txn["payee_name"], str) or not txn["payee_name"].strip():
                    raise DataProcessingError(
