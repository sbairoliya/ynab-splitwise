...
class DuplicateDetector(LoggerMixin):
    ...
    def filter_existing_transactions(
        ...
        for txn in transactions:
            ...
            if import_id not in existing_set:
                ...
                self.logger.info(
                    f"Skipping duplicate transaction with import_id: {import_id}"
                )
    ...
    def _is_content_duplicate(
        self, txn1: Dict[str, Any], txn2: Dict[str, Any], tolerance_days: int
    ) -> bool:
    ...
    def validate_import_ids(self, transactions: List[Dict[str, Any]]) -> None:
        ...
        self.logger.info(
            f"Validated {len(transactions)} transactions with unique import IDs"
        )
    ...
    def get_import_ids_from_transactions(
